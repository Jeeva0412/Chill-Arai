...
function App() {
  ...
  return (
    <div className="flex h-full min-h-screen w-full bg-bg-primary dark:bg-slate-950 text-text-primary dark:text-slate-50 transition-colors duration-300">
      {/* Sidebar Navigation */}
      <nav className="my-6 ml-6 w-20 hover:w-64 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 flex flex-col h-[calc(100vh-3rem)] sticky top-6 z-50 overflow-hidden py-6 text-slate-800 dark:text-slate-200 transition-all duration-300 ease-out group">

        {/* Workspace Selector / Logo */}
        <div className="px-5 mb-10 mt-2 flex items-center whitespace-nowrap">
        ...
        <div className="flex flex-col gap-3 px-3">
          <button
            className={`flex items-center px-4 py-3.5 w-full rounded-lg text-left transition-all duration-300 overflow-hidden ${activeTab === 'dashboard'
              ? 'bg-black text-white dark:bg-white dark:text-black'
              : 'hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-500 hover:text-black dark:hover:text-white'
              }`}
            onClick={() => setActiveTab('dashboard')}
          >
            <Activity size={22} className="shrink-0 transition-transform duration-300" />
          ...
          <button
            className={`flex items-center px-4 py-3.5 w-full rounded-lg text-left transition-all duration-300 overflow-hidden ${activeTab === 'transactions'
              ? 'bg-black text-white dark:bg-white dark:text-black'
              : 'hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-500 hover:text-black dark:hover:text-white'
              }`}
            onClick={() => setActiveTab('transactions')}
          >
            <Wallet size={22} className="shrink-0 transition-transform duration-300" />
            <span className="ml-4 font-semibold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75">
              Transactions
            </span>
          </button>

          <button
            className={`flex items-center px-4 py-3.5 w-full rounded-lg text-left transition-all duration-300 overflow-hidden ${activeTab === 'lending'
              ? 'bg-black text-white dark:bg-white dark:text-black'
              : 'hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-500 hover:text-black dark:hover:text-white'
              }`}
            onClick={() => setActiveTab('lending')}
          >
            <Users size={22} className="shrink-0 transition-transform duration-300" />
            <span className="ml-4 font-semibold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-150">
              Lending Logs
            </span>
          </button>

          <button
            className={`flex items-center px-4 py-3.5 w-full rounded-lg text-left transition-all duration-300 overflow-hidden ${activeTab === 'borrowing'
              ? 'bg-black text-white dark:bg-white dark:text-black'
              : 'hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-500 hover:text-black dark:hover:text-white'
              }`}
            onClick={() => setActiveTab('borrowing')}
          >
            <Download size={22} className="shrink-0 transition-transform duration-300" />
            <span className="ml-4 font-semibold whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-200">
              Borrowing Logs
            </span>
          </button>
        </div>
        ...
        {/* Bottom Actions */}
        <div className="mt-auto px-3 mb-8 w-full">
          <button
            onClick={toggleTheme}
        ...
        {/* Bottom Status (Pushed to absolute bottom) */}
        <div className="absolute pl-6 bottom-4 w-full flex items-center h-8">
      ...
      {/* Main Content Area */}
      <main className="flex-1 p-4 md:p-8 overflow-y-auto w-full max-w-7xl mx-auto relative flex flex-col pt-8">

        {/* Global Header Elements */}
        <div className="w-full flex justify-between items-center mb-10 z-40 bg-transparent border-b border-slate-200 dark:border-slate-800 pb-6">
          <div className="flex flex-col">
            <h2 className="text-3xl md:text-4xl font-extrabold flex items-center gap-2 text-black dark:text-white tracking-tighter transition-colors">
              {activeTab === 'dashboard' && 'Dashboard'}
              {activeTab === 'transactions' && 'Recent Transactions'}
              {activeTab === 'lending' && 'Lending Log'}
              {activeTab === 'borrowing' && 'Borrowing Log'}
            </h2>
            <div className="text-slate-500 dark:text-slate-400 font-medium text-lg mt-1 transition-colors">Welcome, Cherry!</div>
          </div>

          <div className="flex items-center gap-3 md:gap-4">
            {/* Actions */}
            {activeTab === 'transactions' && (
              <button className="flex items-center bg-black hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-200 text-white dark:text-black font-semibold py-2 px-4 rounded-lg transition-colors gap-2" onClick={() => setShowTxForm(true)}>
                <Plus size={18} /> <span className="hidden sm:inline">Add New</span>
              </button>
            )}
            {activeTab === 'lending' && (
              <button className="flex items-center bg-black hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-200 text-white dark:text-black font-semibold py-2 px-4 rounded-lg transition-colors gap-2" onClick={() => setShowLendingForm(true)}>
                <Plus size={18} /> <span className="hidden sm:inline">Add New</span>
              </button>
            )}
            {activeTab === 'borrowing' && (
              <button className="flex items-center bg-black hover:bg-slate-800 dark:bg-white dark:hover:bg-slate-200 text-white dark:text-black font-semibold py-2 px-4 rounded-lg transition-colors gap-2" onClick={() => setShowBorrowForm(true)}>
                <Plus size={18} /> <span className="hidden sm:inline">Add New</span>
              </button>
            )}
        ...
        {activeTab === 'dashboard' && <Dashboard transactions={transactions} lendings={lendings} borrowings={borrowings} />}
