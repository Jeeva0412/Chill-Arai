...
export function TransactionForm({ onSubmit, onCancel, defaultAmount }: { onSubmit: (data: any) => void; onCancel: () => void; defaultAmount?: number }) {
    ...
    return (
        <div className="fixed inset-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-colors">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl p-6 sm:p-8 w-full max-w-md animate-fade-in flex-col gap-6 transition-colors mx-4 sm:mx-0 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="flex-col gap-4">
                    <div className="flex gap-2 mb-6">
                        <button
                            type="button"
                            className={`flex-1 py-2.5 font-semibold text-sm rounded-xl transition-colors border ${type === 'expense' ? 'bg-black dark:bg-white text-white dark:text-black border-black dark:border-white' : 'bg-transparent text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500'}`}
                        ...
                        <button
                            type="button"
                            className={`flex-1 py-2.5 font-semibold text-sm rounded-xl transition-colors border ${type === 'income' ? 'bg-black dark:bg-white text-white dark:text-black border-black dark:border-white' : 'bg-transparent text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500'}`}
                    ...
                    <div className="mb-4 text-slate-800 dark:text-slate-200 transition-colors">
                        ...
                        <input
                            ...
                            placeholder="0.00"
                            className="dark:bg-slate-950 dark:border-slate-700 dark:text-white dark:focus:ring-white dark:focus:border-white rounded-xl transition-colors"
                            required
                        />
                    </div>
                    ...
                    <div className="flex gap-4 mt-8 pt-4 border-t border-slate-100 dark:border-slate-800 transition-colors">
                        <button type="button" className="btn btn-secondary flex-1 dark:bg-transparent dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white rounded-xl transition-colors" onClick={onCancel}>Cancel</button>
                        <button type="submit" className="btn btn-secondary flex-1 dark:bg-white dark:border-slate-700 dark:text-black dark:hover:bg-slate-200 rounded-xl transition-colors">Save Record</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div className="fixed inset-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-colors">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl p-6 sm:p-8 w-full max-w-md animate-fade-in flex-col gap-6 transition-colors mx-4 sm:mx-0 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="flex-col gap-4">
                    ...
                    <div className="mb-4 text-slate-800 dark:text-slate-200 transition-colors">
                        <label className="dark:text-slate-300">Date (Optional)</label>
                        <input
                            type="date"
                            value={date}
                            onChange={(e) => setDate(e.target.value)}
                    ...
                    <div className="flex gap-4 mt-8 pt-4 border-t border-slate-100 dark:border-slate-800 transition-colors">
                        <button type="button" className="btn btn-secondary flex-1 dark:bg-transparent dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white rounded-xl transition-colors" onClick={onCancel}>Cancel</button>
                        <button type="submit" className="btn btn-primary flex-1 dark:bg-white dark:text-black dark:hover:bg-slate-200 rounded-xl transition-colors">Record</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div className="fixed inset-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md flex items-center justify-center z-50 p-4 transition-colors">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl p-6 sm:p-8 w-full max-w-md animate-fade-in flex-col gap-6 transition-colors mx-4 sm:mx-0 max-h-[90vh] overflow-y-auto">
                ...
                <form onSubmit={handleSubmit} className="flex-col gap-4">
                    ...
                    <div className="flex gap-4 mt-8 pt-4 border-t border-slate-100 dark:border-slate-800 transition-colors">
                        <button type="button" className="btn btn-secondary flex-1 dark:bg-transparent dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white rounded-xl transition-colors" onClick={onCancel}>Cancel</button>
                        <button type="submit" className="btn btn-secondary flex-1 dark:bg-white dark:text-black dark:hover:bg-slate-200 rounded-xl transition-colors">Record</button>
                    </div>
                </form>
            </div>
        </div>
